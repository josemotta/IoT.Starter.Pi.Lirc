...
The `lirconsole` program that is running in the container communicates exactly through the `lircd` socket. This is the best way to establish a link using the  docker `volume` concept. Please see below the `Lirc-compose.yml` and `Lirc.dockerfile` files that do all job.
