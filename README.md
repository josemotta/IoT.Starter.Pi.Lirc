...
At the bottom, the `lirconsole` program is running inside the docker container. Communicating  through the `lircd` socket, a docker `volume` is responsible for data sharing between host and container. Please see below the `Lirc-compose.yml` and `Lirc.dockerfile` files that do all job.
