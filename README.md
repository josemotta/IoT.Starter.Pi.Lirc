...
The specification for current mission is to provide infrared (IR) output capability to `IoT.Starter.Pi.Thing` projects. Home gadgets following the lead of universal remote controls has been a desired dream for home automation initiatives of all times. 
...
At the bottom, the `lirconsole` program is running inside the docker container. Communicating  through the `lircd` socket, a docker `volume` enables data sharing between host and container. Following are the `Lirc-compose.yml` and `Lirc.dockerfile` that do all job.
